...
const DeleteLessonModal = ({ setOpen, open, setRows, rowSelectionModel }) => {
    ...
    rowSelectionModel.map(async (id) => {
      try {
        ...
        if (response.data.success) {
          ...
          toast.success(`Lesson with ${id} removed successfully`);
        } else {
          toast.error(`Sorry, removing lesson with ${id} was unsuccessfull.`);
        }
      } catch (error) {
    ...
    >
      ...
      >
        ...
        <Typography sx={{ fontSize: "15px" }}>
          Are you sure that you want to delete selected lesson permanently?
        </Typography>
      </Box>
    </CustomModal>
  );
